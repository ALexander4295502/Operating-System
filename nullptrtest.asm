...
printf(1,"b= %p\n",&b);
   ...
   d:	c7 44 24 04 16 07 00 	movl   $0x716,0x4(%esp)
  14:	00 
...
//b=*np;
printf(1,"b= %p\n",&b);
  23:	e8 88 03 00 00       	call   3b0 <printf>
...
00000304 <translate>:
SYSCALL(translate)
 304:	b8 1b 00 00 00       	mov    $0x1b,%eax
 309:	cd 40                	int    $0x40
 30b:	c3                   	ret    
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 349:	0f b6 92 24 07 00 00 	movzbl 0x724(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 4df:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 4e1:	b8 1d 07 00 00       	mov    $0x71d,%eax
...
free(void *ap)
{
 ...
 5a1:	a1 a0 09 00 00       	mov    0x9a0,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 60f:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 612:	a3 a0 09 00 00       	mov    %eax,0x9a0
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 639:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 63c:	8b 1d a0 09 00 00    	mov    0x9a0,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 679:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 67b:	3b 15 a0 09 00 00    	cmp    0x9a0,%edx
...
morecore(uint nu)
{
 ...
 6ad:	e8 ee fe ff ff       	call   5a0 <free>
  return freep;
 6b2:	8b 15 a0 09 00 00    	mov    0x9a0,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 6d4:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
 6d7:	89 15 a0 09 00 00    	mov    %edx,0x9a0
      return (void*)(p + 1);
 ...
 6ec:	eb e9                	jmp    6d7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6ee:	c7 05 a0 09 00 00 a4 	movl   $0x9a4,0x9a0
 6f5:	09 00 00 
    base.s.size = 0;
 6f8:	ba a4 09 00 00       	mov    $0x9a4,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6fd:	c7 05 a4 09 00 00 a4 	movl   $0x9a4,0x9a4
 704:	09 00 00 
    base.s.size = 0;
 707:	c7 05 a8 09 00 00 00 	movl   $0x0,0x9a8
 70e:	00 00 00 
