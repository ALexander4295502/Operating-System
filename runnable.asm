...
_runnable:     file format elf32-i386
...
main(int argc, char **argv)
{
  ...
  return runnable();
   6:	e8 d1 02 00 00       	call   2dc <runnable>
...
gets(char *buf, int max)
{
 ...
 143:	e8 04 01 00 00       	call   24c <read>
    if(cc < 1)
...
stat(char *n, struct stat *st)
{
 ...
 196:	e8 d9 00 00 00       	call   274 <open>
  if(fd < 0)
...
stat(char *n, struct stat *st)
{
 ...
 1ab:	e8 dc 00 00 00       	call   28c <fstat>
  close(fd);
 ...
 1b3:	89 c6                	mov    %eax,%esi
  close(fd);
 1b5:	e8 a2 00 00 00       	call   25c <close>
  return r;
...
000002e4 <translate>:
SYSCALL(translate)
 2e4:	b8 1b 00 00 00       	mov    $0x1b,%eax
 2e9:	cd 40                	int    $0x40
 2eb:	c3                   	ret    
...
putc(int fd, char c)
{
 ...
 36a:	e8 e5 fe ff ff       	call   254 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 3d6:	e8 79 fe ff ff       	call   254 <write>
...
putc(int fd, char c)
{
 ...
 446:	e8 09 fe ff ff       	call   254 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 463:	e8 ec fd ff ff       	call   254 <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
 ...
 4f0:	e8 5f fd ff ff       	call   254 <write>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
...
putc(int fd, char c)
{
 ...
 520:	e8 2f fd ff ff       	call   254 <write>
...
putc(int fd, char c)
{
 ...
 570:	e8 df fc ff ff       	call   254 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
free(void *ap)
{
 ...
 581:	a1 78 09 00 00       	mov    0x978,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 5c7:	89 10                	mov    %edx,(%eax)
  freep = p;
 5c9:	a3 78 09 00 00       	mov    %eax,0x978
}
 5ce:	5b                   	pop    %ebx
 5cf:	5e                   	pop    %esi
 5d0:	5f                   	pop    %edi
 ...
 5ef:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 5f2:	a3 78 09 00 00       	mov    %eax,0x978
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 619:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 61c:	8b 1d 78 09 00 00    	mov    0x978,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 659:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 65b:	3b 15 78 09 00 00    	cmp    0x978,%edx
...
morecore(uint nu)
{
 ...
 67a:	e8 3d fc ff ff       	call   2bc <sbrk>
  if(p == (char*)-1)
 ...
 68d:	e8 ee fe ff ff       	call   580 <free>
  return freep;
 692:	8b 15 78 09 00 00    	mov    0x978,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 6b4:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
 6b7:	89 15 78 09 00 00    	mov    %edx,0x978
      return (void*)(p + 1);
 ...
 6cc:	eb e9                	jmp    6b7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6ce:	c7 05 78 09 00 00 7c 	movl   $0x97c,0x978
 6d5:	09 00 00 
    base.s.size = 0;
 6d8:	ba 7c 09 00 00       	mov    $0x97c,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6dd:	c7 05 7c 09 00 00 7c 	movl   $0x97c,0x97c
 6e4:	09 00 00 
    base.s.size = 0;
 6e7:	c7 05 80 09 00 00 00 	movl   $0x0,0x980
 6ee:	00 00 00 
