...
runnabletest(void)
{
  ...
  5f:	74 7f                	je     e0 <runnabletest+0xa0>
  61:	89 c3                	mov    %eax,%ebx
{
  ...
  for(i = 0; i < N; i++){ 
     pids[i] = fork();   //fork a new process
  63:	e8 d2 02 00 00       	call   33a <fork>
     if(pids[i] == 0){  // if you're the child, loop forever
  68:	85 c0                	test   %eax,%eax
{
  ...
  6a:	89 c6                	mov    %eax,%esi
     if(pids[i] == 0){  // if you're the child, loop forever
  6c:	74 72                	je     e0 <runnabletest+0xa0>
  ...
  6e:	e8 77 03 00 00       	call   3ea <runnable>
  ...
  a2:	53                   	push   %ebx
  a3:	e8 ca 02 00 00       	call   372 <kill>
  a8:	89 34 24             	mov    %esi,(%esp)
  ab:	e8 c2 02 00 00       	call   372 <kill>
  }

  // wait for all children to finish being killedi
  for(i = 0; i < N; i++){
     wait();
  b0:	e8 95 02 00 00       	call   34a <wait>
  ...
  for(i = 0; i < N; i++){
     wait();
  bf:	e9 86 02 00 00       	jmp    34a <wait>
  ...
  cb:	68 0a 08 00 00       	push   $0x80a
  d0:	6a 01                	push   $0x1
  d2:	e8 c9 03 00 00       	call   4a0 <printf>
  d7:	83 c4 10             	add    $0x10,%esp
  da:	eb c3                	jmp    9f <runnabletest+0x5f>
  ...
  e3:	6a 00                	push   $0x0
  e5:	68 f1 07 00 00       	push   $0x7f1
  ea:	6a 01                	push   $0x1
  ec:	e8 af 03 00 00       	call   4a0 <printf>
  f1:	83 c4 10             	add    $0x10,%esp
  f4:	eb fe                	jmp    f4 <runnabletest+0xb4>
  f6:	66 90                	xchg   %ax,%ax
  f8:	66 90                	xchg   %ax,%ax
  fa:	66 90                	xchg   %ax,%ax
  fc:	66 90                	xchg   %ax,%ax
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 43f:	f7 f6                	div    %esi
 441:	0f b6 92 7c 08 00 00 	movzbl 0x87c(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5b8:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 5bb:	b8 74 08 00 00       	mov    $0x874,%eax
...
free(void *ap)
{
 ...
 697:	89 08                	mov    %ecx,(%eax)
  freep = p;
 699:	a3 44 0b 00 00       	mov    %eax,0xb44
}
 69e:	5b                   	pop    %ebx
 69f:	5e                   	pop    %esi
 6a0:	5f                   	pop    %edi
 ...
 6ba:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 6bd:	a3 44 0b 00 00       	mov    %eax,0xb44
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 72f:	76 3f                	jbe    770 <malloc+0xa0>
    ...
    if(p == freep)
 731:	39 05 44 0b 00 00    	cmp    %eax,0xb44
 ...
 756:	e8 e5 fe ff ff       	call   640 <free>
  return freep;
 75b:	8b 15 44 0b 00 00    	mov    0xb44,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 77c:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
 77f:	89 15 44 0b 00 00    	mov    %edx,0xb44
      return (void*)(p + 1);
 ...
 794:	eb e9                	jmp    77f <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 796:	c7 05 44 0b 00 00 48 	movl   $0xb48,0xb44
 79d:	0b 00 00 
 7a0:	c7 05 48 0b 00 00 48 	movl   $0xb48,0xb48
 7a7:	0b 00 00 
    base.s.size = 0;
 7aa:	b8 48 0b 00 00       	mov    $0xb48,%eax
 7af:	c7 05 4c 0b 00 00 00 	movl   $0x0,0xb4c
 7b6:	00 00 00 
