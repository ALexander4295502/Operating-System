...
int crawl(char *start_url,
	...
	pthread_t cid[download_workers],pid[parse_workers];
	int i;
	for(i = 0; i < download_workers; i++){
		pthread_create(&cid[i],NULL,download_thread,NULL);
	}
	for(i = 0; i < download_workers; i++){
		pthread_create(&pid[i],NULL,parse_thread,NULL);
	}
	pthread_mutex_lock(&w_lock);
	while(work > 0){
		printf("work = %u\n",work);
	...
	printf("out while work = %u\n",work);
	pthread_mutex_unlock(&w_lock);
	pthread_mutex_lock(&w_lock);
        for(i = 0; i< 0; i++){
        	int cjc = pthread_join(cid[i] ,NULL);
		printf("after call pthread_join(cid)\n");
		int pjc = pthread_join(pid[i] ,NULL);
		if(cjc == 0){
			printf("download_thread[%d] is finished \n",i);
		}
		if(pjc == 0){
			printf("parse_thread[%d] is finished \n",i);
		}
        }
	pthread_mutex_unlock(&w_lock);
        pd_free();
