...
_wakeall:     file format elf32-i386
...
main(int argc, char **argv)
{
  ...
  int wakeNUM=wakeall();
   9:	e8 e6 02 00 00       	call   2f4 <wakeall>
  ...
  21:	e8 8a 03 00 00       	call   3b0 <printf>
  exit();
  26:	e8 29 02 00 00       	call   254 <exit>
  2b:	90                   	nop
  2c:	66 90                	xchg   %ax,%ax
  2e:	66 90                	xchg   %ax,%ax
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 349:	0f b6 92 3f 07 00 00 	movzbl 0x73f(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 4df:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 4e1:	b8 38 07 00 00       	mov    $0x738,%eax
...
free(void *ap)
{
 ...
 60f:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 612:	a3 b4 09 00 00       	mov    %eax,0x9b4
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 639:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 63c:	8b 1d b4 09 00 00    	mov    0x9b4,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 679:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 67b:	3b 15 b4 09 00 00    	cmp    0x9b4,%edx
...
morecore(uint nu)
{
 ...
 6ad:	e8 ee fe ff ff       	call   5a0 <free>
  return freep;
 6b2:	8b 15 b4 09 00 00    	mov    0x9b4,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 6d4:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
 6d7:	89 15 b4 09 00 00    	mov    %edx,0x9b4
      return (void*)(p + 1);
 ...
 6ec:	eb e9                	jmp    6d7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6ee:	c7 05 b4 09 00 00 b8 	movl   $0x9b8,0x9b4
 6f5:	09 00 00 
    base.s.size = 0;
 6f8:	ba b8 09 00 00       	mov    $0x9b8,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6fd:	c7 05 b8 09 00 00 b8 	movl   $0x9b8,0x9b8
 704:	09 00 00 
    base.s.size = 0;
 707:	c7 05 bc 09 00 00 00 	movl   $0x0,0x9bc
 70e:	00 00 00 
