...
_runproctest:     file format elf32-i386
...
main(int argc, char *argv[])
{
  ...
  printf(1,"project1tests starting*****\n");
   9:	c7 44 24 04 8e 08 00 	movl   $0x88e,0x4(%esp)
  10:	00 
  11:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  18:	e8 83 04 00 00       	call   4a0 <printf>

  runproctest();
  1d:	e8 1e 00 00 00       	call   40 <runproctest>
  
  printf(1,"project1tests finished*****\n");  //BUG: Print twice, fixing on it.
  22:	c7 44 24 04 ab 08 00 	movl   $0x8ab,0x4(%esp)
  29:	00 
  2a:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  31:	e8 6a 04 00 00       	call   4a0 <printf>
  exit();
  36:	e8 09 03 00 00       	call   344 <exit>
  3b:	66 90                	xchg   %ax,%ax
  3d:	66 90                	xchg   %ax,%ax
  3f:	90                   	nop

00000040 <runproctest>:
...
runproctest(void)
{
  ...
  44:	56                   	push   %esi
  45:	53                   	push   %ebx
  46:	83 ec 6c             	sub    $0x6c,%esp
     int fc=fork();
  49:	e8 ee 02 00 00       	call   33c <fork>
     if(fc==0){
  4e:	83 f8 00             	cmp    $0x0,%eax
  51:	0f 85 b1 00 00 00    	jne    108 <runproctest+0xc8>
       char s[80] = "y";
  57:	8d 7d 9c             	lea    -0x64(%ebp),%edi
  5a:	b9 13 00 00 00       	mov    $0x13,%ecx
  5f:	f3 ab                	rep stos %eax,%es:(%edi)
       int answer=0;
...
runproctest(void)
{
     ...
     if(fc==0){
       char s[80] = "y";
  63:	c7 45 98 79 00 00 00 	movl   $0x79,-0x68(%ebp)
 ...
 gets(s,getsmax);  
  6a:	8d 5d 98             	lea    -0x68(%ebp),%ebx
     if(fc==0){
       ...
       while(1){
if(s[0]=='y'){
  6d:	0f b6 45 98          	movzbl -0x68(%ebp),%eax
  71:	3c 79                	cmp    $0x79,%al
  73:	74 34                	je     a9 <runproctest+0x69>
 fork();
 answer++;
 gets(s,getsmax);
 ...
 printf(1,"there should be %d process runnable, and your answer is %d.\n",answer,runnable());  //if your return value of "runnable()" is same to the answer, your function is passed.
  ...
  75:	3c 6e                	cmp    $0x6e,%al
  77:	0f 84 98 00 00 00    	je     115 <runproctest+0xd5>
 break;  //BUG: after you enter n, however the test function will also ADD A RUNNABLE PROCESS AND TEST YOUR "runnable()", which is being corrected.
}else{
 printf(1,"please enter again: \n");
  7d:	c7 44 24 04 78 08 00 	movl   $0x878,0x4(%esp)
  84:	00 
  85:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  8c:	e8 0f 04 00 00       	call   4a0 <printf>
 gets(s,getsmax);  
  91:	c7 44 24 04 50 00 00 	movl   $0x50,0x4(%esp)
  98:	00 
  99:	89 1c 24             	mov    %ebx,(%esp)
  9c:	e8 8f 01 00 00       	call   230 <gets>
     if(fc==0){
       char s[80] = "y";
       int answer=0;
       ...
       while(1){
if(s[0]=='y'){
  a1:	0f b6 45 98          	movzbl -0x68(%ebp),%eax
  a5:	3c 79                	cmp    $0x79,%al
  a7:	75 cc                	jne    75 <runproctest+0x35>
  printf(1,"do you want to add a runnable process? (y/n)\n");
  a9:	c7 44 24 04 08 08 00 	movl   $0x808,0x4(%esp)
  b0:	00 
 fork();
 answer++;
...
if(s[0]=='y'){
 ...
 fork();
  c0:	e8 77 02 00 00       	call   33c <fork>
 answer++;
 gets(s,getsmax);
  c5:	c7 44 24 04 50 00 00 	movl   $0x50,0x4(%esp)
  cc:	00 
  cd:	89 1c 24             	mov    %ebx,(%esp)
  d0:	e8 5b 01 00 00       	call   230 <gets>
  d5:	8d 76 00             	lea    0x0(%esi),%esi
 while(runnable()==0){} //BUG: it seems that there is sometimes a transition for the state of the process from runnable to sleeping, so we need this loop to avoid this. 
  d8:	e8 0f 03 00 00       	call   3ec <runnable>
  dd:	85 c0                	test   %eax,%eax
  df:	74 f7                	je     d8 <runproctest+0x98>
 printf(1,"there should be %d process runnable, and your answer is %d.\n",answer,runnable());  //if your return value of "runnable()" is same to the answer, your function is passed.
  e1:	e8 06 03 00 00       	call   3ec <runnable>
  e6:	89 74 24 08          	mov    %esi,0x8(%esp)
  ea:	c7 44 24 04 38 08 00 	movl   $0x838,0x4(%esp)
  f1:	00 
  f2:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  f9:	89 44 24 0c          	mov    %eax,0xc(%esp)
  fd:	e8 9e 03 00 00       	call   4a0 <printf>
 102:	e9 66 ff ff ff       	jmp    6d <runproctest+0x2d>
 107:	90                   	nop
}else{
 printf(1,"please enter again: \n");
 gets(s,getsmax);  
 ...
 108:	7e 0b                	jle    115 <runproctest+0xd5>
 10a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
wait();
 110:	e8 37 02 00 00       	call   34c <wait>
}


}
 115:	83 c4 6c             	add    $0x6c,%esp
 118:	5b                   	pop    %ebx
 119:	5e                   	pop    %esi
 11a:	5f                   	pop    %edi
 11b:	5d                   	pop    %ebp
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 439:	0f b6 92 cf 08 00 00 	movzbl 0x8cf(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5cf:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 5d1:	b8 c8 08 00 00       	mov    $0x8c8,%eax
...
free(void *ap)
{
 ...
 6d7:	89 10                	mov    %edx,(%eax)
  freep = p;
 6d9:	a3 70 0b 00 00       	mov    %eax,0xb70
}
 6de:	5b                   	pop    %ebx
 6df:	5e                   	pop    %esi
 6e0:	5f                   	pop    %edi
 ...
 6ff:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 702:	a3 70 0b 00 00       	mov    %eax,0xb70
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 729:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 72c:	8b 1d 70 0b 00 00    	mov    0xb70,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 769:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 76b:	3b 15 70 0b 00 00    	cmp    0xb70,%edx
...
morecore(uint nu)
{
 ...
 79d:	e8 ee fe ff ff       	call   690 <free>
  return freep;
 7a2:	8b 15 70 0b 00 00    	mov    0xb70,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 7dc:	eb e9                	jmp    7c7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7de:	c7 05 70 0b 00 00 74 	movl   $0xb74,0xb70
 7e5:	0b 00 00 
    base.s.size = 0;
 7e8:	ba 74 0b 00 00       	mov    $0xb74,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7ed:	c7 05 74 0b 00 00 74 	movl   $0xb74,0xb74
 7f4:	0b 00 00 
    base.s.size = 0;
 7f7:	c7 05 78 0b 00 00 00 	movl   $0x0,0xb78
 7fe:	00 00 00 
